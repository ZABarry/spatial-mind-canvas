...
| **VR / WebXR** | Enter VR; **`useXrControllerInputBridge`** (mounted as **XrControllerInputBridge** / re-exported as **XrRaycastSelect**) — trigger ray → select nodes, finish connections (target node or ground plane). **`xrSessionGuards`** coordinate graph selection vs world grab so gestures do not fight. **XrWorldGrab** — squeeze grips for world move and two-hand scale (world mode). **Hand-primary sessions** (hands only, no controllers) set `xrHandTrackingPrimary` on the store; Link on the radial is disabled for reliability (controllers stay the full authoring path). **XrWristMenu** — **hand tracking**: show when the **left palm faces you** (hysteresis); **controllers**: toggle with the **left secondary face button** (Y on typical Quest). Global wrist actions route through **xrGlobalMenuActions** (Library, Search, **Version history**, **Bookmarks**, Settings, Undo, Redo, Reset view, Recenter, Reset scale, Cancel, Switch Travel/World, Help, Exit VR, …). **XrNodeRadial** — contextual chips on the selected node (Child, Link, Inspect, Delete, Focus, Recenter; Link hidden when hand-primary). **XrStatusHud** — navigation mode, selection, in-progress gesture hints, save/snapshot feedback, optional **dev metrics** when XR debug HUD is on. Dedicated world-space panels: **XrNodeDetailPanel**, **XrSearchPanel**, **XrMapHistoryPanel**, **XrBookmarksPanel**, **XrSettingsPanel**, **XrHelpHud**; **XrTextPromptHud** for in-VR text (e.g. bookmark names). **XrConfirmHud** for destructive confirms; **XrSessionBridge**; locomotion / vignette / smooth movement in Settings. **Layout** and **ZIP/JSON export** remain **desktop toolbar** workflows (exit VR or use Library). |
