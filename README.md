...
- [ ] **In-headset UI**: flat search, inspector, settings, help, and onboarding are hidden in VR. Use the **wrist menu** for Library, Search, Settings, Undo/Redo, Reset view, Travel/World, **Help**, Exit VR. Use the **node radial** on a selected node (Add child, Link, Inspect, etc.). **Bookmarks** and **Layout** live on the desktop toolbar — exit VR to save views, recall bookmarks, or run layout tools.
