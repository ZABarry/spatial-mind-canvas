...
| **VR / WebXR** | Enter VR; **`useXrControllerInputBridge`** (mounted as **XrControllerInputBridge** / re-exported as **XrRaycastSelect**) — trigger ray → select nodes, finish connections (target node or ground plane). **`xrSessionGuards`** coordinate graph selection vs world grab so gestures do not fight. **XrWorldGrab** — **controllers:** squeeze grips for world move and two-hand scale; **hands-only:** index–thumb pinch (optional **disable** in Settings for workspace moves only). **Hand-primary sessions** set `xrHandTrackingPrimary`; Link on **node actions** stays controller-first (badge in hand mode). **XrWristMenu** — two pages (primary workflow vs **More…** for library/history/settings/**Recall panels**); palm dwell or **Y**. **XrNodeContextActions** — layered strip (primary / secondary / delete) with distance-aware scale. **Head-anchored HTML panels** — damped lane follow via `anchors/` (not rigid HUD snap). **XrStatusHud** — tool/nav, selection, pinch-approach hint, workspace grab phase. Panels: **XrNodeDetailPanel**, **XrSearchPanel**, **XrMapHistoryPanel**, **XrBookmarksPanel**, **XrSettingsPanel**, **XrHelpHud**; **XrTextPromptHud**; **XrConfirmHud**; **XrSessionBridge**; locomotion / comfort in Settings. **Layout** and **ZIP/JSON export** remain **desktop toolbar** workflows (exit VR or use Library). |
