...
| WebXR (Quest-class) | **Shipped** | Travel vs world nav; **two-page wrist menu** (segmented tray, page slide); **node actions** (tiered strip, separated **Delete**); controller grip + optional hand pinch world grab (**Workspace pinch off** comfort toggle); **HTML panels** settle then **world-lock** with **re-home** when the slot drifts; multi-panel **lane nudges**; hand-local pinch / grab / two-hand **cues** (`XrPinchWorkspaceCue`); **status HUD** priority rules to limit stacked copy. |
