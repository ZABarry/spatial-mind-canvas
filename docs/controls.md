...
In VR, the settings panel is a world-space **HTML** surface (`XrSettingsPanel`); **Escape** closes it when focused. Panels spawn in **left / center / near-right / right** lanes: they **track the head slot briefly**, then **stay in world space** until the comfortable slot drifts enough to **re-home** (not continuously glued to the view). When several panels are open, lanes get small **offsets** so stacks read clearly. Opening a panel in VR **re-seeds** its anchor from the current view. If a panel feels lost, use **wrist menu → More… → Recall panels** to re-anchor all lanes.

## XR controllers

- **Enter VR** — Toolbar primary button; confirm in headset/browser.
- **Ray targeting** — Hit tests merge **parallel offset rays** for forgiving selection and apply **stickiness** (distance slack tuned for Quest jitter) to the last successful target per controller. When two hits are almost the same distance, **wrist menu** and **node action strip** surfaces win over the graph (reduces flicker at shallow angles).
...
- **Wrist menu (left)** — Controllers: **secondary face button** (often Y). **Page 1:** Search, Undo, Recenter, Cancel, switch Travel/World, Help, **Settings**, **More…**, **Exit VR** (full-width row). **Page 2:** Library, History, Bookmarks, Redo, Reset view, Reset scale, **Recall panels** (full-width), **« Back**. Page changes use a short **slide** (not flat paging). Tray is **segmented** for depth. With **audio** on, switching pages plays a soft **menu page** cue (distinct from open/close).
- **Node actions** — Primary row **Child**, **Link**, **Inspect** (emphasized); secondary **Focus**, **Recenter** (quieter); **Delete** in a **separated column**. Billboard toward you; distance-aware scale; subtle plate behind the cluster; damped **reveal** on selection change. Hand mode: **Link** shows a **Controllers** badge — use tracked controllers for reliable Link.
- **World grab** — **World** mode: controllers squeeze **grip**; hands-only: **index–thumb pinch** on each hand. One hand: translate. Two: pinch/grip separation scales; opposite forward motion yaws (thresholds reduce accidental coupling). Hand-local **rings**: warm-up near threshold; stronger tint when grab is active; outer hint when **two-hand** adjust is live.
...
- **World grab** — Pinch index–thumb (both hands for scale/yaw) in World mode; same guard rules as controller grips (blocked during menus/modals/link). Optional **Settings → VR → Workspace pinch off (comfort)** if you want controllers only for workspace moves (locks hand workspace manipulation, not UI).
