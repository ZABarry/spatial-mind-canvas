...
| XR wrist menu | Extra 3D UI (text + planes) when open or palm-visible | Same guidance as labels — open only when needed; hand-tracking palm panel adds pose work each frame |
| XR floating panels (search, map history, bookmarks, settings, node detail, help, text prompt) | World-space HTML; **grounded** pose after brief settle; re-seed on open / **Recall panels** | Close when finished; avoid stacking multiple heavy panels; recall if GPU time spikes from many DOM surfaces |
