...
export function XrNodeRadial() {
  ...
  return (
    <group position={n.position as unknown as [number, number, number]}>
      ...
      <mesh position={[-0.75 * n.size, y, 0]} onPointerDown={onAct(() => runNodeInspect())}>
        <boxGeometry args={[0.24, 0.1, 0.04]} />
        <meshStandardMaterial color="#c4b5fd" />
      </mesh>
      <Text position={[-0.75 * n.size, y, 0.03]} fontSize={0.045} color="#0f172a" anchorX="center" anchorY="middle">
        Inspect
      </Text>
    </group>
  )
